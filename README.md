...
```bash
saga-pattern-example-account-transfer$ sam build --use-container
```
...
```bash
saga-pattern-example-account-transfer$ sam logs -n StockCheckerFunction --stack-name saga-pattern-example-account-transfer --tail
```
...
```bash
saga-pattern-example-account-transfer$ pip install -r tests/requirements.txt --user
# unit test
saga-pattern-example-account-transfer$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
saga-pattern-example-account-transfer$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name saga-pattern-example-account-transfer
```
